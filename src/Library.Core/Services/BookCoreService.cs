...
public class BookCoreService(IUnitOfWork unitOfWork, IMapper mapper) : IBookCoreService
{
    ...
    public async Task<ErrorOr<bool>> IsIsbnAvailableAsync (string isbn)
    {
        return !(await IsIsbnExistsAsync (isbn));
    }
    ...
    {
        var found = await unitOfWork.BookRepository.GetAsync(x => x.Isbn.Equals(isbn));
